                            ...
                            uint16_t aN)
{
  for (uint16_t row = 0; row < aM; ++row)
  {
    for (uint16_t col = 0; col < aN; ++col)
    {
      ...
      {
        sum_r += (int32_t)p_in1[0] * p_in2[0];
        sum_r -= (int32_t)p_in1[1] * p_in2[1];
        sum_i += (int32_t)p_in1[1] * p_in2[0];
        sum_i += (int32_t)p_in1[0] * p_in2[1];
...
int main(void)
{
  tpt_mat_cmplx_mult_q15(output_q15, input1_q15, input2_q15, TEST_M, TEST_K,
                          TEST_N);
  dump(output_q15, 20);
